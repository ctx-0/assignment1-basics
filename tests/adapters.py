...
from cs336_basics.transformer import MyLinear, MyEmbedding, RMSNorm, SiLU, SwiGLU
...
def run_linear(
    ...
    l = MyLinear(d_in, d_out, device=in_features.device, dtype=in_features.dtype)
    l.W = weights
...
def run_embedding(
    ...
    el = MyEmbedding(vocab_size, d_model, device=weights.device, dtype=weights.dtype)
    el.E = torch.nn.Parameter(weights)
...
def run_rmsnorm(
    ...
    """
    nl = RMSNorm(d_model, eps)
    nl.W = torch.nn.Parameter(weights)
    return nl.forward(in_features)
